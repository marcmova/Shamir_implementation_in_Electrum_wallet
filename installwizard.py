...
from .seed_dialog import SeedLayout, KeysLayout, DistributeKeysLayout, ResultDistributeKeysLayout
...
class InstallWizard(QDialog, MessageBoxMixin, BaseWizard):
    ...
    def set_layout(self, layout, title=None, next_enabled=True, next_button_visible=True):
        ...
        self.next_button.setVisible(next_button_visible)
        if next_enabled:
    ...
    def exec_layout(
            ...
            focused_widget=None):
        self.set_layout(layout, title, next_enabled, next_button_visible=next_button_visible)
        if focused_widget:
    ...
    def pool_keys_algorithm(self,admin, admin_key, keys):
        P = self.set_P()
        if admin:
            admin_key = int("0x" + admin_key, 16)
        else:
            admin_key = 0
        new_keys = []
        for i in range(len(keys)):
            num_key = int(keys[i][0])
            key = int("0x" + keys[i][1], 16)
            new_keys.append([num_key, key])
        keys = new_keys
        secret_int = Mod(0, P)
        for key1 in keys:
        ...
        try:
            return_value = bytes.fromhex(hex(secret_int._value)[2:]).decode('utf-8')
        except:
            return_value = "Incorrect shares, the seed can not be generated."
        return(return_value)

    def distribute_keys_algorithm(self,admin, mnemonic, num_keys, threshold):
        P = self.set_P()
        random.seed(P)
        distributed_keys = []
        polinomial_factors = []
        ...
        try:
            ...
            return "The format is not correct, the keys can not be generated"
        ...
        for item in distributed_keys:
            result += "\n"
            result += str(item[0]) + "\n"
            result += str(hex(item[1])[2:]) + "\n"
            result += "\n"
        if (admin):
            result += "administrator key:\n"
            result += str(hex(admin_key)[2:]) + "\n"
            result += "\n"
        return result

    def distribute_input(self, title, message, is_seed, options, admin=False):
        slayout = DistributeKeysLayout(
            title=message,
            is_seed=is_seed,
            options=options,
            parent=self,
        ...
        self.exec_layout(slayout, title, next_enabled=True)

        retrieved_text = slayout.get_seed()
        mnemonic = ""
        for i in range(12):
            mnemonic += retrieved_text.split()[i]
            if i!=11:
                mnemonic+=" "


        display_text = self.distribute_keys_algorithm(admin, mnemonic, retrieved_text.split()[-2], retrieved_text.split()[-1])

        result_distribute_layout = ResultDistributeKeysLayout(
            display_text,
            title="The results of the key split are shown on the box below:",
            options=options,
            parent=self,
            config=self.config,
        )
        self.exec_layout(result_distribute_layout, title, next_button_visible=False)

        return slayout.get_seed(), slayout.seed_type, slayout.is_ext
    def pool_input(self, title, message, is_seed, options, admin=False):
        slayout = DistributeKeysLayout(
            title=message,
            is_seed=is_seed,
            options=options,
        ...
        keys = []
        if admin:
            for i in range(int(len(retrieved_text.split())/2)-1):
                key=[]
                key.append(retrieved_text.split()[i * 2])
                key.append(retrieved_text.split()[i * 2 + 1])
                keys.append(key)
        else:
            for i in range(int(len(retrieved_text.split()) / 2)):
                key = []
                key.append(retrieved_text.split()[i * 2])
                key.append(retrieved_text.split()[i * 2 + 1])
                keys.append(key)
        display_text = self.pool_keys_algorithm(admin, retrieved_text.split()[-1], keys)

        result_distribute_layout = ResultDistributeKeysLayout(
            display_text,
            title="The results of the key pool are shown on the box below:",
            options=options,
            parent=self,
            config=self.config,
        )
    ...
    def distribute_keys_dialog(self, run_next, test):
        options = []
        if self.opt_ext:
            options.append('ext')
        if self.opt_bip39:
        ...
        if self.opt_slip39:
            options.append('slip39')
        title = _('Distribute keys')
        message = _('Please enter your seed phrase in order to generate shares, followed by the number of participants and the threshold, separated by spaces.')

        return self.distribute_input(title, message, test, options)
    def distribute_keys_admin_dialog(self, run_next, test):
        options = []
        if self.opt_ext:
            options.append('ext')
        if self.opt_bip39:
        ...
        if self.opt_slip39:
            options.append('slip39')
        title = _('Distribute keys')
        message = _('Please enter your seed phrase in order to generate fragment keys, followed by the number of participants and the threshold, separated by spaces.')

        return self.distribute_input(title, message, test, options, True)

    def pool_keys_dialog(self, run_next, test):
        options = []
        if self.opt_ext:
            options.append('ext')
        if self.opt_bip39:
        ...
        if self.opt_slip39:
            options.append('slip39')
        title = _('Pool keys')
        message = _('Please enter all sequences of key number and key, separated by lines.')

        return self.pool_input(title, message, test, options)

    def pool_keys_admin_dialog(self, run_next, test):
        options = []
        if self.opt_ext:
            options.append('ext')
        if self.opt_bip39:
        ...
        if self.opt_slip39:
            options.append('slip39')
        title = _('Pool keys')
        message = _('Please enter all sequences of key number and key, separated by lines, put the administrator key on the last position.')

        return self.pool_input(title, message, test, options, True)
